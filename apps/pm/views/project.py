...
class ProjectViewSet(ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if self.request.data.get('owner', False):
            ...
            if self.request.data.get('owner') != project_obj.owner:
                ...
                serializer.save(updated_by=self.request.user.username, members=list(members))
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            return JsonResponse(data=paginated_data, msg='success', success=True, status=status.HTTP_200_OK)
        serializer = self.get_serializer(queryset, many=True)
        return JsonResponse(data=serializer.data, msg='success', success=True, status=status.HTTP_200_OK)
