...
class UserCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        exclude = ('password', 'avatar', 'first_name', 'last_name',
                   'is_staff', 'groups', 'user_permissions', 'last_login', 'is_active', 'is_superuser')
...
class UserThinRetrieveSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'avatar', 'nick_name', 'position')
...
class UserRetrieveSerializer(serializers.ModelSerializer):
    date_joined = serializers.DateTimeField(format='%Y-%m-%d %H:%M:%S', read_only=True, help_text='加入时间')
    department = DepartmentBaseRetrieveSerializer(many=False, read_only=True)
...
class UserResetPasswordSerializer(serializers.ModelSerializer):
    old_password = serializers.CharField(label='旧密码', help_text='旧密码', write_only=True, required=True,
                                         allow_blank=False)
    new_password = serializers.CharField(min_length=8,
                                         ...
                                         allow_blank=False,
                                         error_messages={
                                             'min_length': '新密码长度不能小于8',
                                             'max_length': '新密码长度不能大于128', })
