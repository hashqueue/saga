    ...
    if response is not None:
        ...
        if isinstance(exc, ValidationError):
            ...
            if isinstance(response.data, list):
                response.data = {'success': False, 'message': response.data[0], 'data': None}
            return response
        if 'detail' in response.data:
            response.data = {'success': False, 'message': response.data.get('detail'), 'data': None}
        else:
            # 未知错误
            response.data = {'success': False, 'message': str(response.data), 'data': None}
        return response
    return response
